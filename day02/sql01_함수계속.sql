/* 단일행함수 */
--
SELECT CONCAT('Hello', 'Oracle')	-- 한행에 한열만 출력되는 값을 = 스칼라(Scalar)값
  FROM dual;

-- 인덱스가 1부터, 일반 프로그래밍 언어의 배열이 0부터 시작하는 것과의 차이 
-- SUBSTR(변환할 값, 인덱스, 길이) - substring() 함수와 동일, 파이썬 슬라이싱
-- -인덱스 - 뒤에서부터 위치
...
-- MOD() 나누기 나머지값 - 파이썬 % 연산과 동일
-- POWER() 파이썬 math.pow(), numpy.power(), **, 2^10 승수계산 동일
SELECT 786.5427 AS res1
	 , round(786.5427) AS round0 -- 소수점없이
	 , round(786.5472, 1) AS round1 -- 소수점 1자리 까지
	 , round(786.5472, 3) AS round2 -- 소수점 3자리 까지
	 , ceil(786.5427) AS cdilRes
