...
-- 문1. Emoloyee 테이블에서 사원번호, 이름(이름 성 합쳐서 표시), 급여, 업무, 입사일, 상사의 사원번호로 출력하시오(107행)
SELECT employee_id
...
-- 문2. Emoloyee 테이블에서 모든 사원의 이름(last_name)과 연봉을 '이름: 1 year salary = $연봉'형식으로
-- 출력하고, 컬럼명을 1 Year Salary로 변경하시오(107행)
SELECT last_name || ': 1 year salary =' || to_char(salary * 12, '$999,999,999') AS "1 Year Salary"
  FROM employees;
...
--	 , salary + round(salary * 0.153) AS "Increased Salary(참조)"
  FROM employees
...
--	 , (salary * 12) + (salary * NVL(commission_pct, 0)) AS "Annual Salary"
	 , (salary * NVL(commission_pct, 1)) * 12 AS "Annual Salary"
